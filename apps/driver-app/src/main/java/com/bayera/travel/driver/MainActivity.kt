            ...
            MaterialTheme {
                Surface(modifier = Modifier.fillMaxSize()) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        Text("Bayera Partner App", style = MaterialTheme.typography.headlineMedium)
                        Spacer(modifier = Modifier.height(20.dp))
                        Text("Driver Dashboard: Waiting for jobs...")
                    }
                }
            }
        }
...
fun ErrorScreen(error: String) {
    Column(modifier = Modifier.fillMaxSize().background(Color.Black).padding(16.dp).verticalScroll(rememberScrollState())) {
        Text("DRIVER APP CRASHED", color = Color.Red, style = MaterialTheme.typography.headlineSmall)
